...
def create_category(category: schemas.ProjectCategoryCreate, db: Session = Depends(get_db)):
...
def update_milestone(milestone_id: int, milestone: schemas.MilestoneBase, db: Session = Depends(get_db)):
    db_milestone = db.query(models.Milestone).filter(models.Milestone.id == milestone_id).first()
    if not db_milestone:
        raise HTTPException(status_code=404, detail="Milestone not found")
    for k, v in milestone.dict().items():
